...
### 24.11 Both remaining suspects eliminated, and the premise verified

**`0x2CDFD` is the exact mirror of `0x2D04C`.** Encode opens by copying the block's
**R** (`es:[bx+4]`) into the LFSR operand, running the LFSR and folding the result
into L (`0x2CE48`), then storing old-R to L -- where decode opens on **L** and folds
into R. Its second stage uses the same constant `0x803425C3` with rotate `(2i) & 0x1F`
ascending, against decode's `2*(5-i)`. Proper inverses. The games call service `0x3D`
(50 sites against 6 for `0x3C`), so decode is what runs and decode is what was
modelled. **Suspect 2 out.**

**The out-pointer writes are as assumed.** Traced instruction by instruction: LFSR #1
operates on `[bp-0x8]` (`0x2D14A`), LFSR #2 on `[bp-0xc]` (`0x2D249`), and `0x2D282`
writes `[bp-0xa]:[bp-0xc]` -- result #2 -- to `dest[0]`, and `[bp-0x6]:[bp-0x8]` --
result #1 -- to `dest[4]`. So `D` is the second LFSR result and `acc` the first,
which is what the schedule was fed. **Suspect 1 out.**

**And the known-plaintext premise is now verified rather than argued.** Strip each
generation's header layer -- LCG with the per-name key for 2000, LCG with `0x12345`
for 2001 -- and the first 128 bytes are **byte-identical for 40 of 40 pictures**.
The archives hold the same images, so the plaintext really is the plaintext.

### 24.12 State: the model is verified everywhere it has been checked, and still fails

Everything listed in § 24.10 has now been checked and matches the code. The solve
still does not verify block 1. That is an uncomfortable place to leave it and it is
recorded as such rather than papered over: some assumption in the chain is still
wrong and it is not any of the ones enumerated so far.

The least-examined pieces remaining, for whoever picks this up:

- **Feistel A's rotation schedule.** `0x2D07E` computes `(5 - i) * 5` via `imul`, read
  once and never tested in isolation. Encode's mirror should use `5 * i` ascending;
  confirming that against `0x2CDFD`'s tail would validate both.
- **Whether `0x2EB26` is entered once per 4 KB chunk.** The chunk arithmetic in
  `0x17237` is the game's file loop; that the decode service is called once per chunk
  with exactly that size is inferred from it, not observed. A trace with a breakpoint
  on the service entry would settle it.
- **Whether the archives were produced by this code at all.** The pictures are shared
  with the 2000 release, so the 2001 archive may have been generated by a build tool
  rather than by the runtime's own encode path.
