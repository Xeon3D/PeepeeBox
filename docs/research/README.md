...
| `35-funnet.md` | **What was on the other end of the call, and a server that stands in for it.** PPP, then WATTCP, then one FTP session -- the whole of fun.net. A Dutch 2001 MASTERS cabinet kept its account (`ftp.nl.funsys.com`, `pp` / `92tx45dt`), its tournament state, 2 MB of downloaded rankings, and a log of the **twenty-nine updates it was sent down the telephone between 2001 and 2002, games included** -- and it went on dialling the dead service until 2016. [`fun.net-server`](https://github.com/Xeon3D/fun.net-server) is the stand-in; the script grammar and the `.upd` cipher are what remain |
| `36-igo5-buttons.md` | **I.G.O. 5's garbled buttons are HaspDecodeData.** Two button faces in the 2005 hi-score archive are enciphered whole, and the game hands the library 4 KB at a time where it takes 8 or 16 bytes -- refused before the wire, on a real cabinet too |
| `37-igo3-boots.md` | **I.G.O. 3 boots, and its games pass their dongle check.** Five faults, each behind the last, found by running the release's own HASP library under unicorn: the liveness probe, read-time queries, bit-7 Microwire record reads, an anti-replay sweep gate, and `\FOTO\GAMESTAT.OLD` -- a table of real answers in four EncodeData modes, which the device now answers from |
