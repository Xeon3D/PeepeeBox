...
static inline uint32_t sec_get(uint32_t i, const uint8_t *t, int wide)
{
    const uint32_t idx = wide ? ((i >> 2) & 7u) : ((i >> 2) & 0x0eu);

    return (t[idx] >> ((31u - i) & 7u)) & 1u;
}
...
{
    static const uint8_t CT[8]   = { 0xd2, 0x46, 0x3a, 0x28, 0xcf, 0xf9, 0x62, 0xd6 };
    static const uint8_t WANT[8] = { 0x0a, 0x05, 0x01, 0x08, 0x00, 0x00, 0x00, 0x00 };
    ...
    if (argc < 2) {
        fprintf(stderr, "need the dump path\n");
        return 2;
    }
    f = fopen(argv[1], "rb");
    if (!f) {
    ...
    if (argc > 2) {
        lo = hi = (int) strtol(argv[2], NULL, 0);
    } else {
        lo = 0;
        hi = (int) n - 8;
    }
    printf("dump %s (%zu bytes), table offsets %d..%d\n", argv[1], n, lo, hi);
        ...
        for (int wide = 0; wide < 2; wide++) {
            for (int cm = 0; cm < 256; cm++) {
                for (int civ = 0; civ < 256; civ++) {
                    ...
                    prepare(&k, dump + off, (uint8_t) cm, (uint8_t) civ, wide);
                    decode_block(CT, out, &k);
                    for (int i = 0; i < 8; i++)
                        m += (out[i] == WANT[i]);
                    if (m == 8) {
                        printf("  *** HIT  table@0x%02X wide=%d column_mask=%02X civ=%02X\n",
                               off, wide, cm, civ);
                        hits++;
                    } else if (m > best) {
                        best = m;
                        printf("  best so far %d/8  table@0x%02X wide=%d cm=%02X civ=%02X -> "
    ...
    printf("done: %d exact hit(s), best partial %d/8\n", hits, best);
    return hits ? 0 : 1;
}
